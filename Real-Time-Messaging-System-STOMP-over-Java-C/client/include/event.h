...
private:
    ...
    std::string summaryeventOwnerUser;
...
public:
    ...
    std::string summaryToString(int counter) const;
};
...
struct names_and_events {
    ...
    names_and_events() : channel_name(""), events() {}

    names_and_events(const std::string &channel, const std::vector<Event> &evts)
        : channel_name(channel), events(evts) {}
};
