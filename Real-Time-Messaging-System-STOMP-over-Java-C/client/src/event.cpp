...
Event::Event(std::string channel_name, std::string city, std::string name, int date_time,
    ...
    : channel_name(channel_name), city(city), name(name),
      date_time(date_time), description(description), general_information(general_information)
{
    eventOwnerUser="";
    summaryeventOwnerUser="";
}
...
std::string Event::summaryToString(int counter) const {
    std::ostringstream body;

    // Add the event's basic details
    body << "Report_" << counter << ":\n";
    body << "city: " << city << "\n";
...
std::string Event::toString() const {
    ...
    if (!eventOwnerUser.empty()) {
        body << "Event Owner: " << eventOwnerUser << "\n";
    }

    return body.str();
}

Event::Event(const std::string &frame_body): channel_name(""), city(""), //build event from string
